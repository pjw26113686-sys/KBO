...
kbo/
  ...
  model/poisson.py   # λ 추정 → 득점분포 → 3종 마켓 확률 + 예측구간
  ...
  crawler/, scheduler/  # 2차 구현 예정 (인터페이스 스텁)
scripts/simulate.py  # CLI 엔트리포인트
tests/               # pytest
```
...
# N경기 시뮬레이션 후 검증 리포트 출력
python scripts/simulate.py --games 5000 --seed 42

# 결과를 SQLite 에 적재(스키마 검증)
python scripts/simulate.py --games 1000 --db sim.sqlite
...
- 실제 Statiz/KBO/MyKBO 크롤링 (robots.txt·rate-limit·개인용도 준수)
- 과거 시즌 실데이터 적재 + 파라미터 그리드서치 튜닝
- APScheduler 경기별 잡(시작 30분 전) + 디스코드 웹훅 발송
- 날씨·음이항 분포·ML 고도화
